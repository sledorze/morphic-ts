...
export const ioTsNonStrictObjectInterpreter: ModelAlgebraObject1<URI> = {
  interface: <A>(props: PropsKind1<'IOTSType', A>, name: string, _config?: Partial<ObjectInterfaceConfig>) => {
    const interfaceType: t.InterfaceType<IOTypes<A>> = t.type(projectField(props)('type'), name)
    return new IOTSType<A>(interfaceType)
  },
  partial: <A>(props: PropsKind1<'IOTSType', A>, name: string, _config?: Partial<ObjectPartialConfig>) => {
