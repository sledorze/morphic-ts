...
export const jsonSchemaObjectInterpreter: ModelAlgebraObject1<URI> = {
  interface: props => new JsonSchema(ObjectTypeCtor(false, record.toArray(props).map(([k, v]) => tuple(k, v.schema)))),
  partial: props => new JsonSchema(ObjectTypeCtor(true, record.toArray(props).map(([k, v]) => tuple(k, v.schema))))
}
